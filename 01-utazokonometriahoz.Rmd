...
lattice::xyplot( score ~ tsratio, data = SimData, xlab = "Tanár:diák arány", ylab = "Teszteredmény")
```
...
```{r, echo=FALSE}
lattice::xyplot( score ~ tsratio, data = SimData, subset = SES=="Jó SES", xlab = "Tanár:diák arány", ylab = "Teszteredmény")
```

Érdekes! Itt fordított a kapcsolat. Na de mi a helyzet a rossz szocioökonómiai státuszú osztályokban? Íme:

```{r, echo=FALSE}
lattice::xyplot( score ~ tsratio, data = SimData, subset = SES=="Rossz SES", xlab = "Tanár:diák arány", ylab = "Teszteredmény")
```
...
```{r, echo=FALSE}
lattice::xyplot( score ~ tsratio, groups = SES, data = SimData, auto.key = list( columns=2 ), xlab = "Tanár:diák arány", ylab = "Teszteredmény" )
```
...
## Történetünk második szála: az ökonometriai modellek és a regresszió
...
```{r,echo=FALSE}
lattice::contourplot( f ~ tsratiogrid + scoregrid, data = JointDensity, xlab = "Tanár:diák arány",
                      xlim = c( 0.04, 0.065 ), ylab = "Teszteredmény", zlab = "Sűrűség" )
```
...
```{r, echo=FALSE}
lattice::contourplot( f ~ tsratiogrid + scoregrid, data = JointDensity, xlab = "Tanár:diák arány",
                      xlim = c( 0.04, 0.065 ), ylab = "Teszteredmény", zlab = "Sűrűség", panel = function( ... ) {
                        lattice::panel.contourplot( ... )
                        lattice::panel.abline( v = 0.055, col = "red" )
                      } )
```
...
lattice::xyplot( f ~ scoregrid, data = CondDensity, type = "l", xlab = "Teszteredmény", ylab = "Sűrűség" )
```
...
lattice::xyplot( f ~ scoregrid, data = CondDensity, type = "l", xlab = "Teszteredmény", ylab = "Sűrűség",
                 panel = function( ... ) {
                   ...
                   lattice::panel.abline( v = sum(CondDensity$f*CondDensity$scoregrid), col = "orange" )
                 })
```
...
lattice::contourplot( f ~ tsratiogrid + scoregrid, data = JointDensity, xlab = "Tanár:diák arány",
                      xlim = c( 0.04, 0.065 ), ylab = "Teszteredmény", zlab = "Sűrűség", panel = function( ... ) {
                        lattice::panel.contourplot( ... )
                        lattice::panel.points( 0.055, sum(CondDensity$f*CondDensity$scoregrid), col = "orange", pch = 4 )
                      } )
```
...
lattice::contourplot( f ~ tsratiogrid + scoregrid, data = JointDensity, xlab = "Tanár:diák arány",
                      xlim = c( 0.04, 0.065 ), ylab = "Teszteredmény", zlab = "Sűrűség", panel = function( ... ) {
                        lattice::panel.contourplot( ... )
                        lattice::panel.points( seq( 0.04, 0.07, 0.001 ), 
                                               sapply( seq( 0.04, 0.07, 0.001 ), function( tsrat ) {
...
Ebből talán még világosabban látszik a korábbi állítás: hogy többváltozós normális eloszlásnál tehát speciálisan a regressziófüggvény lineáris lesz.

Érdemes megnézni, hogy a még áttekinthető kétváltozós esetben ez az általános eredmény mire specializálódik:
